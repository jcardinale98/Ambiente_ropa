$(document).ready(function () {
  ...
  $("#formActualizarContrasenna").submit(function (evento) {
    evento.preventDefault();
...
function ActualizarPerfil() {
  ...
  const correoElectronico = $("#txtCorreoElectronico").val().trim();

  if (nombre === "") {
    MostrarMensaje("Debe ingresar el nombre.", false);

    return;
  }

  if (correoElectronico === "") {
    MostrarMensaje("Debe ingresar el correo electrónico.", false);
  ...
  $.ajax({
    ...
    error: function (xhr) {
      ...
      MostrarMensaje("No se pudo actualizar el perfil.", false);
    },
...
function ActualizarContrasenna() {
  const contrasennaActual = $("#txtContrasennaActual").val();

  const nuevaContrasenna = $("#txtNuevaContrasenna").val();

  const confirmarContrasenna = $("#txtConfirmarContrasenna").val();

  if (contrasennaActual === "") {
    MostrarMensaje("Debe ingresar la contraseña actual.", false);

    return;
  }

  if (nuevaContrasenna === "") {
    MostrarMensaje("Debe ingresar la nueva contraseña.", false);
  ...
  if (confirmarContrasenna === "") {
    MostrarMensaje("Debe confirmar la nueva contraseña.", false);

    return;
  }

  if (nuevaContrasenna !== confirmarContrasenna) {
    MostrarMensaje("La confirmación de la contraseña no coincide.", false);
  ...
  const boton = $("#btnActualizarContrasenna");
  ...
  $.ajax({
    ...
    complete: function () {
      ...
      boton.html('<i class="fa-solid fa-key"></i> ' + "Actualizar contraseña");
    },
  });
}
